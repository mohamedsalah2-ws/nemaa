// --- بيانات المنتجات ---
const productsDB = [
    { id: 1, name: "سوبر نيتروجين", cat: "نمو خضري", categoryCode: "growth", img: "https://images.unsplash.com/photo-1585314062340-f1a5a7c9328d?w=800", desc: "أقوى سماد للنمو الخضري.", usage: "رش ورقي", specs: "نيتروجين 46%" },
    { id: 2, name: "فوسفو روتس", cat: "جذور", categoryCode: "roots", img: "https://images.unsplash.com/photo-1574943320219-553eb213f72d?w=800", desc: "جذور قوية وتزهير كثيف.", usage: "ري بالتنقيط", specs: "فسفور 50%" },
    { id: 3, name: "بوتاسيوم كينج", cat: "إثمار", categoryCode: "fruit", img: "https://images.unsplash.com/photo-1592982537447-6f2a6a0c7c18?w=800", desc: "تحجيم وجودة للثمار.", usage: "بعد العقد", specs: "بوتاسيوم 40%" },
    { id: 4, name: "ميكرو باور", cat: "عناصر", categoryCode: "elements", img: "https://images.unsplash.com/photo-1464226184884-fa280b87c399?w=800", desc: "علاج نقص العناصر.", usage: "عند الاصفرار", specs: "حديد + زنك" }
];

document.addEventListener("DOMContentLoaded", function() {
    
    // 1. حقن النافبار والفوتر (Component Injection)
    const navbarHTML = `
    ...
    // 4. تعبئة المنتجات (Home Slider & Product Page Logic)
    const swiperWrapper = document.querySelector('.swiper-wrapper');
    if (swiperWrapper) { // If on Home Page
        ...
        new Swiper(".mySwiper", { slidesPerView: 1, spaceBetween: 25, loop: true, autoplay: { delay: 3000 }, pagination: { clickable: true }, breakpoints: { 640: { slidesPerView: 2 }, 1024: { slidesPerView: 3 } } });
    }

    // 5. Product Filtering Logic (Store Page)
    const filterBtns = document.querySelectorAll('.filter-btn');
    const productGrid = document.querySelector('.products-grid'); // Container in products.html
    
    if (filterBtns.length > 0 && productGrid) { // If on Store Page
    ...
    if (document.getElementById('p-name') && productId) {
        ...
        if (product) {
            ...
            tabDesc.addEventListener('click', () => { tabDesc.classList.add('active'); tabUsage.classList.remove('active'); contentArea.innerText = product.desc + "\n\n" + product.specs; });
            tabUsage.addEventListener('click', () => { tabUsage.classList.add('active'); tabDesc.classList.remove('active'); contentArea.innerText = product.usage; });
        }
    }
...
function createProductCard(product) {
    return `
    <div class="product-card" data-category="${product.categoryCode}">
...
function initParticles() {
    ...
    class Particle {
        ...
        update() { this.x += this.vx; this.y += this.vy; if (this.x < 0 || this.x > w) this.vx *= -1; if (this.y < 0 || this.y > h) this.vy *= -1; if(mouse.x != null) { let dx = mouse.x - this.x; let dy = mouse.y - this.y; let distance = Math.sqrt(dx*dx + dy*dy); if(distance < 150) { this.x -= dx/20; this.y -= dy/20; } } }
    ...
    function animate() { ctx.clearRect(0, 0, w, h); for(let i=0; i<particles.length; i++) { particles[i].update(); particles[i].draw(); for(let j=i; j<particles.length; j++) { let dx = particles[i].x - particles[j].x; let dy = particles[i].y - particles[j].y; let distance = Math.sqrt(dx*dx + dy*dy); if(distance < 100) { ctx.beginPath(); ctx.strokeStyle = `rgba(16, 185, 129, ${0.1 * (1 - distance/100)})`; ctx.lineWidth = 0.5; ctx.moveTo(particles[i].x, particles[i].y); ctx.lineTo(particles[j].x, particles[j].y); ctx.stroke(); } } } requestAnimationFrame(animate); }
